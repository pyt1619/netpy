...
def new_mac_random(selection):
	...
	print(net_interface)
def random_mac(): # рандомный мак адресс
	if question():
		if not "eth" in net_interface[0]:
			os.system("ifconfig "+net_interface[0]+" down")
			os.system("macchanger -r "+net_interface[0])
			os.system("ifconfig "+net_interface[0]+" up")
		else:
			os.system("macchanger -r "+net_interface[0])

def rych_mac(): # ручной мак
	if len(text_1.get()) == 17 and question():
		if not "eth" in net_interface[0]:
			...
			os.system("macchanger --mac "+text_1.get().lower()+" "+net_interface[0])
def rand_mac_pr(): # Сменить рандомно для производителя
	if question():
		...
		if not "eth" in net_interface[0]:
			...
			os.system("macchanger --mac "+net_interface[1].lower()+" "+net_interface[0])
def rych_mac_pr(): # ручной мак для производителя
	if len(text_2.get()) == 17 and question():
		if not "eth" in net_interface[0]:
			...
			os.system("macchanger --mac "+text_2.get().lower()+" "+net_interface[0])
def reboot_mac(): # восстановить мак
	if question():
		if not "eth" in net_interface[0]:
			os.system("ifconfig "+net_interface[0]+" down")
...
text = Text(tab2,width=35, height=10, wrap=WORD, font="Arial 9")
text.pack()
text.place(relx=0.81, rely=0.325,anchor="c", bordermode=OUTSIDE)


os.system("ifconfig | grep eth > doc3.txt")
os.system("ifconfig | grep wlan >> doc3.txt")

my_filex = open("doc3.txt")
ifconfig = my_filex.read()
my_filex.close()

text.configure(state=NORMAL)
text.delete(1.0, END)# очстка 
text.insert(1.0, ifconfig)
